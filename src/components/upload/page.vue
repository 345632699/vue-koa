<template>
  <div class="page">
    ...
    <div class="file-panel">
      <h2>文件列表</h2>
      <div class="file-list">
        <ul class="file-item" :class="`file-${file.id}`" v-for="(file,index) in fileList" v-bind:key="index">
          ...
          <li class="file-operate">
            <a title="开始" @click="resume(file)"><i class="iconfont icon-control-play"></i>开始</a>
            <a title="暂停" @click="stop(file)"><i class="iconfont icon-video-pause"></i>暂停</a>
            <a title="移除" @click="remove(file)"><i class="iconfont icon-close-big"></i>删除</a>
          </li>
